...
def overlay_on_background(frame, mask, bg_image):
    mask_3c = np.dstack([mask] * 3)
    foreground = cv2.bitwise_and(frame, frame, mask=mask)
    background = cv2.bitwise_and(bg_image, bg_image, mask=cv2.bitwise_not(mask))
    combined = cv2.add(foreground, background)
    return combined

def process_video(video_path, bg_image_path, output_path):
    ...
    while cap.isOpened():
        ...
        resized_bg = resize_background(bg_image, width, height)
...
if __name__ == "__main__":
    ...
    parser.add_argument("bg_image", type=str, help="Path to the background image.")
    args = parser.parse_args()
    
    # Generate output path based on input filenames
    base_name = os.path.basename(args.video).split('.')[0]
    bg_name = os.path.basename(args.bg_image).split('.')[0]
    output_path = f"{base_name}_{bg_name}_output.mp4"
    
    process_video(args.video, args.bg_image, output_path)
