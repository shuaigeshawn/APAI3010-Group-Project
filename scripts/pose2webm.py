...
def save_video_webm(video_tensor, path, fps=30):
    """Save video tensor as WebM format with alpha channel support"""
    video_numpy = (video_tensor * 255).byte().cpu().numpy().transpose(0, 2, 3, 1)
    
    container = av.open(path, mode='w')
    stream = container.add_stream('vp9', rate=fps)
    stream.width = video_numpy.shape[2]
    stream.height = video_numpy.shape[1]
...
def main():
    ...
    for ref_image_path in config["test_cases"].keys():
        ...
        for pose_video_path in config["test_cases"][ref_image_path]:
            ...
            # 确保输入图片是PNG格式且具有alpha通道
            ref_image_pil = Image.open(ref_image_path).convert("RGBA")
            ...
            save_video_webm(
                video,
                f"{save_dir}/{ref_name}_{pose_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}.webm",
